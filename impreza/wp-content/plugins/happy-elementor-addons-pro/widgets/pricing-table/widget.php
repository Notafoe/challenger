...
class Pricing_Table extends Base {
	...
	protected function register_content_controls() {
		...
		$this->add_control(
			'button_attributes',
			[
				'label' => __( 'Attributes', 'happy-addons-pro' ),
				'type' => Controls_Manager::TEXTAREA,
				'dynamic' => [
					'active' => true,
				],
				'placeholder' => __( 'key|value', 'happy-addons-pro' ),
				'description' => sprintf( __( 'Set custom attributes for the Pricing Button. Each attribute in a separate line. Separate attribute key from the value using %s character.', 'happy-addons-pro' ), '<code>|</code>' )
			]
		);
	...
	protected function render() {
		...
		if ( ! empty( $settings['button_attributes'] ) ) {
			$attributes = explode( "\n", $settings['button_attributes'] );
			$black_list = [ 'class', 'href', 'data-elementor-setting-key', 'data-elementor-inline-editing-toolbar', 'onload', 'onfocus', 'onblur', 'onchange', 'onresize', 'onmouseover', 'onmouseout', 'onkeydown', 'onkeyup', 'onerror' ];
			foreach ( $attributes as $attribute ) {
				if ( ! empty( $attribute ) ) {
					$attr = explode( '|', $attribute, 2 );
					if ( ! isset( $attr[1] ) ) { $attr[1] = ''; }
					if ( ! in_array( strtolower( $attr[0] ), $black_list ) ) {
						$this->add_render_attribute( 'button_text', trim( $attr[0] ), trim( $attr[1] ) );
					}
				}
			}
		}
