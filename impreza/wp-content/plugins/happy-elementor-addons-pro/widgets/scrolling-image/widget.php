	...
	protected function render() {
		...
		<div <?php $this->print_render_attribute_string('wrapper');?>>
			<div <?php $this->print_render_attribute_string('container');?>>
				...
				//image link
				$image = wp_get_attachment_image_url($value['image']['id'], $settings['thumbnail_size']);
				if (!$image) { $image = $value['image']['url'];}
				//anchor link
				$repeater_key = 'scroll_items' . $index;
				$this->set_render_attribute($repeater_key, 'class', ['ha-scrolling-image-item','elementor-repeater-item-'.$value['_id']]);
				$item_tag = 'div';
				if ($value['link']['url']) {
					$item_tag = 'a';
					$this->set_render_attribute($repeater_key, 'href', esc_url($value['link']['url']));
					if (!empty($value['link']['is_external'])) {
						$this->set_render_attribute($repeater_key, 'target', '_blank');
					}
					if (!empty($value['link']['nofollow'])) {
						$this->set_render_attribute($repeater_key, 'rel', 'nofollow');
					}
				}
				?>
				...
				<?php
				if (!empty($image)) {
					printf('<figure class="ha-scrolling-image"><img src="%s" alt="%s" title="%s"></figure>', esc_url($image), esc_attr($value['title']), esc_attr($value['title']));
				}
				?>
				<?php
