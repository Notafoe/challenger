...
function us_get_used_in_locations( $post_ID, $show_no_results = FALSE ) {
	...
	$us_portfolio_label = ! empty( $usof_options['portfolio_rename'] )
		? wp_strip_all_tags( $usof_options['portfolio_label_name'], TRUE )
		: __( 'Portfolio', 'us' );
	...
	if ( class_exists( 'woocommerce' ) ) {
		$usage_query = "
			...
			WHERE
				`tm`.`meta_key` = 'content_product_cat_id'
				AND `tm`.`meta_value` = ". (int) $post_ID ."
			LIMIT 0, 100;
		";
		foreach ( $wpdb->get_results( $usage_query ) as $usage_result ) {
			$result .= '<div><strong>' . us_translate( 'Product Category', 'woocommerce' ) . ' > ' . $usage_result->name . '</strong> (<a href="term.php?taxonomy=product_cat&tag_ID=' . $usage_result->term_id . '&post_type=product" target="_blank" rel="noopener">' . us_translate( 'Edit' ) . '</a>)</div>';
		}
	}
