...
if ( ! function_exists( 'us_get_page_area_id' ) ) {
	...
	function us_get_page_area_id( $area ) {
		...
		} elseif ( class_exists( 'woocommerce' ) AND is_tax( 'product_cat' ) AND $area == 'content' ) {
			$area_id = get_term_meta( get_queried_object_id(), $area . '_product_cat_id', TRUE );

			if ( ! $area_id OR $area_id == '__defaults__' ) {
				foreach ( array( 'tax_product_cat_id', 'shop_id' ) as $option ) {
					$area_id = us_get_option( $area . '_' . $option, '__defaults__' );
					if ( $area_id AND $area_id !== '__defaults__' ) {
						break;
					}
				}
			}
...
if ( ! function_exists( 'us_output_design_css' ) ) {
	...
	function us_output_design_css( $custom_posts = [] ) {
		...
		foreach ( array( 'header', 'titlebar', 'content', 'footer' ) as $area ) {
			...
			if ( ! empty( $area_id ) ) {
				$post = get_post( (int) $area_id );
				if ( $area !== 'header' ) {
					$posts[] = $post;
				} else {
					// Get current page block header
		 ...
		 * @return array
		 */
		$func_parse_page_block = function( $post, $current_level = 1 ) use( &$func_parse_page_block, &$posts ) {
			$output = array();
			if ( $current_level > 15/* Max level */ ) {
				return $output;
			}
				...
				if ( preg_match_all( $page_block_pattern, $post->post_content, $matches ) ) {
					foreach ( us_arr_path( $matches, '3', array() ) as $atts ) {
						...
						if ( $next_post instanceof WP_Post AND strrpos( $next_post->post_content, 'us_page_block' ) !== FALSE ) {
							$output = array_merge( $output, $func_parse_page_block( $next_post, ++$current_level ) );
						}
					}
				}
			}
