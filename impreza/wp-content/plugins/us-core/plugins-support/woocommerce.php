...
function us_maybe_change_woocommerce_template_path() {
	...
	if ( is_shop() AND us_get_option( 'content_shop_id', '' ) != '' ) {
		...
		if ( us_get_option( 'content_tax_' . $current_tax . '_id', '__defaults__' ) != '__defaults__' ) {
			...
			$content_product_cat_id = get_term_meta( get_queried_object_id(), 'content_product_cat_id' );
			if( isset( $content_product_cat_id[0] ) AND is_numeric( $content_product_cat_id[0] ) ) {
				$custom_content = TRUE;
			}
		}
...
if ( ! function_exists( 'us_posts_clauses' ) ) {
	...
	function us_posts_clauses( $args, $wp_query ) {
		...
		if (
			...
			$args['orderby'] = rtrim( $args['orderby'] ) . ', wp_posts.ID ' . ( strrpos( $args['orderby'] , 'ASC' ) !== FALSE  ? 'ASC' : 'DESC' );
		}
		return $args;
	}
...
/*
*************** Adjust HTML markup for WooCommerce category ***************
*/
if ( ! function_exists( 'us_product_cat_fields' ) ) {
	add_action( 'product_cat_add_form_fields', 'us_product_cat_fields', 1 );
	add_action( 'product_cat_edit_form_fields', 'us_product_cat_fields', 9 );
	/**
	 * Add custom field to WC Term
	 *
	 * @param object $term Term object
	 * @return void
	 */
	function us_product_cat_fields( $term ) {
		...
		if (
			is_string( $term ) AND 'product_cat' == $term
			OR ! $content_product_cat_id = get_term_meta( $term->term_id, 'content_product_cat_id', TRUE )
		) {
			$content_product_cat_id = '__defaults__';
		}
		?>
		<tr class="form-field term-display-type-wrap">
			<th scope="row" valign="top">
				<label for="content_product_cat_id">
					<?php esc_html_e( 'Content template', 'us' ) ?>
				</label>
			</th>
			<td>
				<select id="content_product_cat_id" name="content_product_cat_id" class="postform">
					<?php foreach( $options as $value => $name ): ?>
						<option value="<?php esc_html_e( $value ) ?>" <?php selected( $value, $content_product_cat_id ) ?>>
							<?php esc_html_e( $name ) ?>
						</option>
					<?php endforeach ?>
				</select>
				<p class="description">
					<?php echo $misc['content_description']; ?>
				</p>
			</td>
		</tr>
		<?php
...
if ( ! function_exists( 'us_save_product_cat_fields' ) ) {
	add_action( 'created_term', 'us_save_product_cat_fields', 10, 3 );
	add_action( 'edit_term', 'us_save_product_cat_fields', 10, 3 );
	/**
	 ...
	 */
	function us_save_product_cat_fields( $term_id, $tt_id = '', $taxonomy = '' ) {
		if ( isset( $_POST['content_product_cat_id'] ) AND 'product_cat' === $taxonomy ) {
			update_term_meta( $term_id, 'content_product_cat_id', esc_attr( $_POST['content_product_cat_id'] ) );
		}
	}
}
