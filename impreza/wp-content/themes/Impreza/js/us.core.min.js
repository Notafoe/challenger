...
this.scrollTo(target);this.lastScroll=currentTime}}.bind(this))}}},_countPosition:function(section){section.top=section.$section.offset().top-this._canvasTopOffset;if((($us.header.headerTop===undefined&&$us.header.settings.is_hidden===undefined)||$us.header.headerTop>0)&&!$us.header.autoHide&&$us.header.applyHeaderTop){section.top=section.top-parseInt($us.header.scrolledOccupiedHeight)}
...
this.width=0;this.tabWidths=[];this.tabHeights=[];this.tabTops=[];this.tabLefts=[];this.isTogglable=(this.$container.usMod('type')==='togglable');this.isStretched=this.$tabsList.hasClass('stretch');this.minWidth=0;this.count=this.$tabs.length;if(this.count===0){return}
...
this.contents[index].css('display','none').attr('aria-expanded','false').slideDown(this.options.duration,this._events.contentChanged);if($us.canvas.winWidth<768&&this.headerClicked==!0){var newTop=this.headers[0].offset().top;for(var i=0;i<index;i ++){newTop+=this.headers[i].outerHeight()}
$us.scroll.scrollTo(newTop,!0);this.headerClicked=!1}
this.$sections.removeClass('active');this.sections[index].addClass('active')}else if(this.curLayout==='ver'){if(this.contents[this.active[0]]!==undefined){this.contents[this.active[0]].css('display','none').attr('aria-expanded','false')}
